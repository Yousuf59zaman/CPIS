<template>
  <div class="contents">
    <!-- Mobile Overlay -->
    <div v-if="isOpen" class="fixed inset-0 bg-black/50 z-[60] lg:hidden transition-opacity duration-300"
    ...
    <div
      ...
      <div
        ...
        <img src="@/assets/images/CPIS_Logo.png" alt="CPIS Logo"
        ...
        <div
          ...
          <div class="w-full h-full rounded-full border-[10px] border-dashed border-gray-400 rotate-12"></div>
        </div>
      </div>

      <!-- Navigation Menu -->
      <nav class="flex-1 px-[20px] 2xl:px-[26px] pt-6 2xl:pt-8 flex flex-col gap-[12px] 2xl:gap-[18px]">
        <router-link v-for="item in navItems" :key="item.name" :to="item.path" @click="$emit('close')"
    ...
    <button v-if="isOpen" @click="$emit('close')"
