...
const highlightCards = [
  { label: 'Claim ID', value: 'CPIS-2026-001', emphasis: 'text-[#171a1f]' },
  { label: 'Voucher No', value: 'DV-2026-045', emphasis: 'text-[#171a1f]' },
  { label: 'Total Amount (USD)', value: '50,000.00', emphasis: 'text-[#171a1f] text-[28px] md:text-[40px] leading-none' },
]
...
<template>
  <main class="flex flex-col gap-[20px] bg-[#f4f5f7] p-[20px] 2xl:p-[24px]">
    <div class="flex flex-wrap items-start justify-between gap-[12px]">
      <h1 class="text-[24px] md:text-[30px] font-medium text-[#202224]">Payment Authorisation</h1>
      ...
      <div class="flex flex-wrap items-center justify-end gap-[10px]">
        ...
        >
          ...
          <ChevronRight class="h-[10px] w-[10px]" />
          <span class="text-[#6b6b73]">Payment Authorization</span>
        </div>
      </div>
    </div>
    ...
    <section class="rounded-[12px] bg-white p-[12px] shadow-[8px_8px_72px_0px_rgba(0,0,0,0.05)]">
      <div class="grid grid-cols-1 gap-[10px] md:grid-cols-2">
        ...
        >
          <p class="text-[12px] text-[#202224]">{{ field.label }}</p>
          <div class="flex min-h-[34px] items-center rounded-[6px] border border-[#ececec] bg-white px-[12px]">
            <span class="text-[12px] font-medium text-[#6a6c71]">{{ field.value }}</span>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="grid grid-cols-1 gap-[18px] md:grid-cols-3">
      ...
      >
        <div class="mb-[12px] h-[70px] border-l-[4px] border-[#f3d6aa] pl-[14px]">
          ...
          <p class="mt-[10px] font-medium text-[#171a1f]" :class="card.emphasis">{{ card.value }}</p>
        </div>
      </article>
    </section>
    ...
    <section class="rounded-[12px] bg-white p-[18px] shadow-[8px_8px_72px_0px_rgba(0,0,0,0.05)]">
      <div class="mb-[18px] flex items-center gap-[10px]">
        ...
        <h2 class="text-[18px] md:text-[20px] font-medium text-[#202224]">Voucher Line Details</h2>
      </div>
        ...
        <table class="w-full min-w-[760px]">
          <thead>
            <tr class="border-b border-[#efefef] bg-[#fafafa]">
              ...
              <th class="px-[12px] py-[12px] text-left text-[10px] font-medium uppercase text-[#4f4f58]">Line Item</th>
              ...
              <th class="px-[12px] py-[12px] text-right text-[10px] font-medium uppercase text-[#4f4f58]">Amount</th>
            </tr>
          </thead>
          ...
          <tbody>
            <tr v-for="line in voucherLines" :key="line.id" class="border-b border-[#f3f3f3]">
              ...
              <td class="px-[12px] py-[14px] text-[12px] text-[#5d6168]">{{ line.id }}</td>
              <td class="px-[12px] py-[14px] text-[12px] text-[#5d6168]">{{ line.accountCode }}</td>
              <td class="px-[12px] py-[14px] text-[12px] text-[#5d6168]">{{ line.description }}</td>
              <td class="px-[12px] py-[14px] text-right text-[12px] text-[#5d6168]">{{ line.amount }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <section class="rounded-[12px] bg-white p-[18px] shadow-[8px_8px_72px_0px_rgba(0,0,0,0.05)]">
      <div class="mb-[18px] flex items-center gap-[10px]">
        <div class="h-[22px] w-[4px] rounded-br-[5px] rounded-tr-[5px] bg-[#275227]"></div>
        <h2 class="text-[18px] md:text-[20px] font-medium text-[#202224]">Legal Recommendation</h2>
      </div>

      <div class="grid grid-cols-1 gap-[24px] xl:grid-cols-[240px_minmax(0,1fr)]">
        <div>
          <p class="mb-[12px] text-[12px] font-medium text-[#202224]">Selection</p>
          <div class="space-y-[12px]">
            ...
            >
              ...
              />
              <div>
                <p class="text-[14px] text-[#202224]">{{ option.label }}</p>
                <p class="text-[12px] text-[#7d7e87]">{{ option.description }}</p>
              </div>
            </label>
          </div>
        </div>
        ...
        <div>
          <label for="authorisation-comments" class="mb-[12px] block text-[12px] font-medium text-[#202224]">
          ...
          <textarea
            ...
            class="min-h-[92px] w-full rounded-[8px] border border-[#ececec] bg-white px-[14px] py-[12px] text-[12px] text-[#4f4f58] outline-none transition focus:border-[#275227]"
          ></textarea>
