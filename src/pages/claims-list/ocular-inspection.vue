...
<template>
  <main class="flex flex-col gap-[16px] 2xl:gap-[20px] p-[20px] 2xl:p-[24px]">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-[16px]">
      ...
      <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm p-[20px]">
        <div class="flex items-center justify-between mb-[14px]">
          <h2 class="text-[18px] font-semibold text-[#171a1f]">CP-00101</h2>
          <button class="flex items-center gap-[6px] px-[12px] py-[6px] border border-[#e5e7eb] rounded-[6px] text-[12px] text-[#374151] hover:bg-[#f3f4f6] transition-colors">
      ...
      <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm p-[20px]">
        <div class="flex items-center justify-between mb-[14px]">
          <h2 class="text-[16px] font-semibold text-[#171a1f]">Inspection Scheduling</h2>
          <span class="px-[10px] py-[4px] bg-[#f3f4f6] text-[#374151] text-[11px] font-medium rounded-full">Inspection schedule</span>
        </div>
        <div class="flex flex-col gap-[10px]">
          <div v-for="field in [
            ...
            <div class="flex items-start gap-[10px]">
              <p class="text-[11px] text-[#6b7280] w-[120px] shrink-0 pt-[8px]">{{ field.label }}</p>
          ...
          <button class="w-full py-[11px] mt-[6px] bg-[#d97706] hover:bg-[#b45309] text-white text-[13px] font-semibold rounded-[8px] transition-colors flex items-center justify-center gap-[6px]">
    ...
    <div>
      <h2 class="text-[16px] font-semibold text-[#171a1f] mb-[12px]">Inspection Assessment</h2>
      <div class="flex border-b border-[#e5e7eb] mb-[16px]">
        <button
          @click="activeTab = 'valid'"
          class="px-[18px] py-[8px] text-[13px] font-medium border-b-2 transition-colors"
      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-[16px]">
        ...
        <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm overflow-hidden">
          <div class="flex items-center gap-[10px] px-[20px] py-[14px] border-b border-[#f5f5f5]">
            ...
            <h3 class="text-[15px] font-semibold text-[#171a1f]">Findings</h3>
          </div>
          <table class="w-full">
            <thead>
            ...
            <tbody>
              <tr v-for="doc in findings" :key="doc.name" class="border-b border-[#f8f9fa] hover:bg-[#fafafa] transition-colors">
                ...
                <td class="px-[12px] py-[10px] text-[11px] text-[#374151]">{{ doc.name }}</td>
                <td class="px-[12px] py-[10px] text-[11px] text-[#374151]">{{ doc.by }}</td>
                <td class="px-[12px] py-[10px] text-[11px] text-[#374151]">{{ doc.date }}</td>
        ...
        <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm p-[20px]">
          <div class="flex items-center gap-[10px] mb-[14px]">
            <div class="w-[4px] h-[18px] bg-[#224e22] rounded-full"></div>
            <h3 class="text-[15px] font-semibold text-[#171a1f]">Action Panel</h3>
          </div>
          <div class="mb-[14px]">
            <label class="block text-[11px] text-[#6b7280] mb-[5px]">Barangay</label>
            <div class="flex items-center gap-[8px]">
              <div class="flex-1 px-[10px] py-[8px] border border-[#e5e7eb] rounded-[6px] bg-[#f9fafb] flex items-center justify-between cursor-pointer">
                <span class="text-[12px] text-[#374151]">Barangay</span>
