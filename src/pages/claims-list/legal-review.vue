...
<template>
  <main class="flex flex-col gap-[16px] 2xl:gap-[20px] p-[20px] 2xl:p-[24px]">
    ...
    <div class="flex items-start justify-between flex-wrap gap-[8px]">
      <h1 class="text-[22px] 2xl:text-[24px] font-semibold text-[#171a1f]">Legal Review</h1>
      <div class="flex items-center gap-[20px] text-[11px] text-[#6b7280]">
        <span>Session: <span class="font-medium text-[#374151]">LR-2025-0023</span></span>
        <span class="w-px h-[14px] bg-[#e5e7eb]"></span>
        <span>Audit Ref: <span class="font-medium text-[#374151]">AR-9901</span></span>
      </div>
    </div>
      ...
      <div v-if="showDocs" class="px-[20px] pb-[16px]">
        <div class="space-y-[8px]">
          <div v-for="doc in docs" :key="doc.name" class="flex items-center gap-[8px]">
            <button class="flex-1 flex items-center justify-between text-[12px] text-[#374151] border border-[#e5e7eb] rounded-[6px] px-[10px] py-[7px] hover:bg-[#f3f4f6]">
              📄 {{ doc.name }} <ChevronDown class="w-[12px] h-[12px] text-[#6b7280]" />
            </button>
            <select class="text-[11px] border border-[#e5e7eb] rounded-[5px] px-[8px] py-[7px] text-[#374151] outline-none min-w-[100px]">
              <option>{{ doc.category }}</option>
            </select>
            <select class="text-[11px] border border-[#e5e7eb] rounded-[5px] px-[8px] py-[7px] text-[#374151] outline-none min-w-[80px]">
    ...
    <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm p-[20px]">
      <div class="flex items-center gap-[10px] mb-[14px]">
        <div class="w-[4px] h-[18px] bg-[#224e22] rounded-full"></div>
        <h2 class="text-[16px] font-semibold text-[#171a1f]">Legal Findings</h2>
      </div>
      <table class="w-full">
        <thead>
          <tr class="border-b border-[#f0f0f0] bg-[#fafafa]">
            ...
            <th class="px-[12px] py-[9px] text-[10px] font-medium text-[#6b7280] uppercase text-left">Voucher No.</th>
        ...
        <tbody>
          <tr v-for="r in legalFindings" :key="r.voucher" class="border-b border-[#f8f9fa] hover:bg-[#fafafa]">
            <td class="px-[12px] py-[10px] text-[12px] text-[#374151]">{{ r.date }}</td>
            <td class="px-[12px] py-[10px] text-[12px] text-[#374151]">{{ r.voucher }}</td>
            <td class="px-[12px] py-[10px] text-[12px] text-[#374151]">{{ r.file }}</td>
            <td class="px-[12px] py-[10px] text-[12px] text-[#374151]">{{ r.by }}</td>
    ...
    <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm p-[20px]">
      <div class="flex items-center gap-[10px] mb-[14px]">
        <div class="w-[4px] h-[18px] bg-[#224e22] rounded-full"></div>
        <h2 class="text-[16px] font-semibold text-[#171a1f]">Payment Records</h2>
      </div>
      <table class="w-full">
        <thead>
          <tr class="border-b border-[#f0f0f0] bg-[#fafafa]">
            <th class="px-[12px] py-[9px] text-[10px] font-medium text-[#6b7280] uppercase text-left">Date</th>
            <th class="px-[12px] py-[9px] text-[10px] font-medium text-[#6b7280] uppercase text-left">Reference</th>
            <th class="px-[12px] py-[9px] text-[10px] font-medium text-[#6b7280] uppercase text-left">Description</th>
            <th class="px-[12px] py-[9px] text-[10px] font-medium text-[#6b7280] uppercase text-left">Amount</th>
            <th class="px-[12px] py-[9px] text-[10px] font-medium text-[#6b7280] uppercase text-left">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="r in paymentRecords" :key="r.ref" class="border-b border-[#f8f9fa] hover:bg-[#fafafa]">
            <td class="px-[12px] py-[10px] text-[12px] text-[#374151]">{{ r.date }}</td>
            <td class="px-[12px] py-[10px] text-[12px] text-[#374151]">{{ r.ref }}</td>
            <td class="px-[12px] py-[10px] text-[12px] text-[#374151]">{{ r.desc }}</td>
            <td class="px-[12px] py-[10px] text-[12px] font-medium text-[#374151]">{{ r.amount }}</td>
            <td class="px-[12px] py-[10px]">
              <div class="flex gap-[6px]">
                <button class="text-[#6b7280] hover:text-[#374151]"><Eye class="w-[13px] h-[13px]" /></button>
                <button class="text-[#6b7280] hover:text-[#374151]"><Download class="w-[13px] h-[13px]" /></button>
    ...
    <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm p-[20px]">
      ...
      <textarea rows="4" placeholder="Enter detailed legal notes and reasoning for the recommendation..." class="w-full text-[12px] border border-[#e5e7eb] rounded-[8px] px-[12px] py-[10px] outline-none focus:border-[#224e22] resize-none text-[#374151] placeholder-[#9ca3af]"></textarea>
    </div>
