...
<template>
  <main class="flex flex-col gap-[16px] 2xl:gap-[20px] p-[20px] 2xl:p-[24px] font-['Poppins']">
    ...
    <div class="flex items-start justify-between flex-wrap gap-[8px]">
      <h1 class="text-[30px] leading-[45px] font-medium text-[#202224]">Inspection Review And Endorsement</h1>
      <nav class="text-[16px] leading-[24px] font-light text-[#A7A6A6] flex items-center gap-[6px]">
        <span class="text-[#A7A6A6]">Dashboard</span><span class="text-[#A7A6A6]">›</span><span class="text-[#A7A6A6]">Claims List</span><span class="text-[#A7A6A6]">›</span>
        <span class="text-[#171A1F]">Inspection Review</span>
      </nav>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-[16px]">
      ...
      <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm p-[20px]">
        <div class="flex items-center gap-[10px] mb-[10px]">
          <div class="w-[4px] h-[18px] bg-[#224e22] rounded-full"></div>
          <h2 class="text-[30px] leading-[45px] font-normal text-[#234F23]">Inspection Assessment</h2>
        </div>
        <div class="flex border-b border-[#e5e7eb] mb-[12px]">
          <button v-for="t in ['valid','discrepant','reviewed']" :key="t" @click="activeTab = t as any"
            class="px-[12px] py-[7px] text-[20px] leading-[30px] font-normal border-b-2 capitalize transition-colors"
            :class="activeTab === t ? 'border-[#224e22] text-[#224e22]' : 'border-transparent text-[#6b7280]'">
            {{ t }}
          </button>
        </div>
        <!-- Findings -->
        <p class="text-[20px] leading-[28px] font-normal text-[#171A1F] mb-[6px]">Findings</p>
        <ul class="text-[18px] leading-[27px] font-light text-[#232323] space-y-[4px] mb-[12px]">
        ...
        <!-- Damage Categories -->
        <p class="text-[20px] leading-[28px] font-normal text-[#171A1F] mb-[6px]">Damage Categories</p>
        <div class="grid grid-cols-2 gap-[6px] mb-[12px]">
          <label v-for="cat in categories" :key="cat.label" class="flex items-center gap-[6px] text-[18px] leading-[27px] font-light text-[#171A1F]">
            <input type="checkbox" :checked="cat.checked" class="w-[12px] h-[12px] accent-[#224e22]" />
            {{ cat.label }}
          </label>
        </div>
        <!-- Estimated Severity -->
        <p class="text-[20px] leading-[28px] font-normal text-[#171A1F] mb-[6px]">Estimated Severity</p>
        <div class="space-y-[8px]">
          <div v-for="doc in attachments" :key="doc.name" class="flex items-center gap-[8px]">
            <button class="flex-1 flex items-center justify-between text-[18px] leading-[27px] font-light text-[#171A1F] border border-[#e5e7eb] rounded-[5px] px-[8px] py-[5px] hover:bg-[#f3f4f6]">
              📄 {{ doc.name }} <ChevronDown class="w-[12px] h-[12px] text-[#6b7280]" />
            </button>
            <select class="text-[18px] leading-[27px] font-light border border-[#e5e7eb] rounded-[5px] px-[6px] py-[5px] text-[#171A1F] outline-none">
    ...
    <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm p-[20px]">
      <div class="flex items-center gap-[10px] mb-[14px]">
        <div class="w-[4px] h-[18px] bg-[#224e22] rounded-full"></div>
        <h2 class="text-[30px] leading-[45px] font-normal text-[#234F23]">Inspection Review Checklist</h2>
      </div>
      <table class="w-full">
        <thead>
          <tr class="border-b border-[#f0f0f0] bg-[#fafafa]">
            <th class="px-[12px] py-[9px] text-[18px] leading-[27px] font-normal text-[#202224] uppercase text-left">Checklist Item</th>
        ...
        <tbody>
          <tr v-for="row in checklist" :key="row.item" class="border-b border-[#f8f9fa] hover:bg-[#fafafa]">
            <td class="px-[12px] py-[10px] text-[18px] leading-[27px] font-normal text-[#202224] opacity-80">{{ row.item }}</td>
            <td class="px-[12px] py-[10px]">
              <span class="px-[8px] py-[2px] rounded-full text-[15px] leading-[22px] font-light text-[#1B1F25] flex items-center justify-center h-[23px]"
                :class="row.statusColor === 'green' ? 'bg-[#d1fae5]' : 'bg-[#fed7aa]'">
                {{ row.status }}
              </span>
            </td>
            <td class="px-[12px] py-[10px]">
              <button class="flex items-center gap-[4px] text-[16px] leading-[24px] font-medium text-[#171A1F] border border-[#e5e7eb] rounded-[5px] px-[8px] py-[4px] hover:bg-[#f3f4f6]">
    ...
    <div class="bg-white rounded-[12px] border border-[#f0f0f0] shadow-sm p-[20px]">
      <div class="flex items-center gap-[10px] mb-[14px]">
        <div class="w-[4px] h-[18px] bg-[#224e22] rounded-full"></div>
        <h2 class="text-[30px] leading-[45px] font-normal text-[#234F23]">Decision and Endorsement Panel</h2>
      </div>
      <div class="flex flex-col gap-[10px] mb-[16px]">
        <label class="flex items-start gap-[10px] p-[12px] border rounded-[8px] cursor-pointer transition-colors"
          ...
          <div>
            <p class="text-[20px] leading-[28px] font-normal text-[#171A1F]">Endorse for Legal Evaluation</p>
            <p class="text-[16px] leading-[20px] font-light text-[#808080]">Inspection findings are complete and verified. Forward to legal team for review.</p>
          </div>
        </label>
        <label class="flex items-start gap-[10px] p-[12px] border rounded-[8px] cursor-pointer transition-colors"
          ...
          <div>
            <p class="text-[20px] leading-[28px] font-normal text-[#171A1F]">Require Further Investigation</p>
            <p class="text-[16px] leading-[20px] font-light text-[#808080]">Additional inspection or documentation is needed before endorsement.</p>
          </div>
        </label>
      </div>
      <p class="text-[20px] leading-[28px] font-normal text-[#171A1F] mb-[6px]">Endorsement Comments</p>
      <textarea rows="4" placeholder="Add comments or notes regarding the endorsement decision..." class="w-full text-[18px] leading-[27px] font-light border border-[#e5e7eb] rounded-[8px] px-[12px] py-[10px] outline-none focus:border-[#224e22] resize-none text-[#171A1F] placeholder-[#9ca3af]"></textarea>
    </div>
